...
def draw_slot(screen,player):
    ...
    bar_x = 0.5 * config.width - (bar_width /2 ) # Centered horizontally
    ...
    if timer > 0:
        font = pygame.font.SysFont(None, int(0.02 * config.width))  # Smaller font size
        text_color = (255, 255, 255)  # White text
        time_remaining = f"{timer / fps:.1f}"  # Format as seconds
        text_surface = font.render(time_remaining, True, text_color)
        text_rect = text_surface.get_rect(center=(bar_x + bar_width / 2, bar_y - bar_height * 2))
        screen.blit(text_surface, text_rect)


def draw_level_up_bar(screen,player):
