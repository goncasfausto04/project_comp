...
def handle_resolution(screen):
    """Handles the resolution of the screen."""
    # Set up the font
    font_path = os.path.join(base_path, "extras", "Pixeboy.ttf")
    font = pygame.font.Font(font_path, 50)
    text = font.render("Resolution: 720x720", True, (255, 255, 255))
    text_rect = text.get_rect(center=(360, 360))

    # Display the resolution text
    screen.blit(text, text_rect)
    pygame.display.flip()
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_g:
                pygame.display.set_mode((1440, 720))
                return
